...
| `pandas-ta` 0.4.71b0 (last upstream release; the repo went private) pins `numba==0.61.2`, which caps `numpy<2.3`; Dependabot's weekly run fails on numpy, pandas, numba, and vectorbt, all stuck at their floors. Its only runtime consumer is ML feature engineering, which the in-house indicator core already covers | `pyproject.toml`, `maverick/backtesting/strategies/ml/feature_engineering.py` | dependencies |
