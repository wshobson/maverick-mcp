...
def test_portfolio_risk_analysis():
    ...
    try:
        ...
        print(f"✅ Success! Current price: ${result.get('current_price')}, Risk level: {result.get('risk_level')}")
        print(f"   Position sizing: ${result.get('position_sizing', {}).get('suggested_position_size')}")
...
async def test_research_empty_results():
    ...
    try:
        ...
        if hasattr(agent, 'search_providers') and len(agent.search_providers) > 0:
            print(f"✅ Research agent created with {len(agent.search_providers)} search providers!")
            ...
            if providers_configured > 0:
                print(f"✅ {providers_configured} search providers have API keys configured")
                return True
            else:
...
def main():
    ...
    if all(results):
        ...
        print("2. ✅ Portfolio risk analysis works (DataFrame validation & column case)")
