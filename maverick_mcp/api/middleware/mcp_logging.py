...
from maverick_mcp.utils.logging import get_logger, request_id_var, tool_name_var, request_start_var
...
class MCPLoggingMiddleware(Middleware if MIDDLEWARE_AVAILABLE else object):
    ...
    async def on_call_tool(self, context: MiddlewareContext, call_next) -> Any:
        ...
        try:
            ...
            execution_time = time.time() - start_time
            error_msg = f"Tool {tool_name} timed out after 25 seconds"
            ...
            execution_time = time.time() - start_time
            self._log_tool_call_error(request_id, tool_name, e, execution_time, arguments)
            raise
    ...
    async def on_read_resource(self, context: MiddlewareContext, call_next) -> Any:
        ...
        try:
            ...
            execution_time = time.time() - start_time
            print(f"❌ RESOURCE ERROR: {resource_uri} ({execution_time:.2f}s) - {type(e).__name__}: {str(e)}")
            raise
    ...
    def _log_tool_call_success(self, request_id: str, tool_name: str, result: Any, execution_time: float):
        ...
        if self.include_payloads and result is not None:
            try:
                result_str = json.dumps(result)[:self.max_payload_length] if not isinstance(result, str) else result[:self.max_payload_length]
        ...
        status_icon = "🟢" if execution_time < 5.0 else "🟡" if execution_time < 15.0 else "🟠"
        print(f"{status_icon} TOOL SUCCESS: {tool_name} [{request_id[:8]}] {execution_time:.2f}s")
    
    def _log_tool_call_timeout(self, request_id: str, tool_name: str, execution_time: float):
        """Log tool timeout."""
        log_data = {
            "request_id": request_id,
            "direction": "outgoing", 
        ...
        print(f"⏰ TOOL TIMEOUT: {tool_name} [{request_id[:8]}] {execution_time:.2f}s (exceeded 25s limit)")
    
    def _log_tool_call_error(self, request_id: str, tool_name: str, error: Exception, execution_time: float, arguments: dict):
        ...
        print(f"❌ TOOL ERROR: {tool_name} [{request_id[:8]}] {execution_time:.2f}s - {type(error).__name__}: {str(error)}")
...
class ToolExecutionLogger:
    ...
    def error(self, step_name: str, error: Exception, message: str = None):
        ...
        print(f"  ❌ {self.tool_name} -> {step_name} ERROR: {type(error).__name__}: {str(error)}")
...
def add_mcp_logging_middleware(
    ...
    logger.info("MCP logging middleware setup completed", extra={
        "include_payloads": include_payloads,
        "max_payload_length": max_payload_length,
        ...
        def decorator(func):
            ...
            async def wrapper(*func_args, **func_kwargs):
                ...
                try:
                    ...
                    execution_time = time.time() - start_time
                    print(f"❌ TOOL ERROR: {func_name} ({execution_time:.2f}s) - {type(e).__name__}: {str(e)}")
                    raise
