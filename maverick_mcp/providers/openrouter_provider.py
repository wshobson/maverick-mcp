...
class ModelProfile(BaseModel):
    ...
    cost_per_million_input: float = Field(description="Cost per million input tokens in USD")
    cost_per_million_output: float = Field(description="Cost per million output tokens in USD")
    ...
    temperature: float = Field(default=0.3, description="Default temperature for this model")
...
MODEL_PROFILES = {
    ...
    "google/gemini-2.5-deep-think": ModelProfile(
        model_id="google/gemini-2.5-deep-think",
        name="Gemini 2.5 Deep Think",
        provider="google",
        context_length=1000000,
        cost_per_million_input=5.0,
        cost_per_million_output=20.0,
        speed_rating=5,  # Slower due to deep thinking
        quality_rating=10,
        best_for=[
            TaskType.DEEP_RESEARCH,
...
class OpenRouterProvider:
    ...
    def get_llm(
        ...
        final_temperature = temperature if temperature is not None else model_profile.temperature
    ...
    def _select_model(
        ...
        for profile in candidates:
            ...
            avg_cost = (profile.cost_per_million_input + profile.cost_per_million_output) / 2
    ...
    def recommend_models_for_workload(
        ...
        for task_type, frequency in workload.items():
            ...
            # Calculate cost
            input_cost = (avg_input_tokens * frequency * model.cost_per_million_input) / 1_000_000
            output_cost = (avg_output_tokens * frequency * model.cost_per_million_output) / 1_000_000
