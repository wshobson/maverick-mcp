...
async def test_timeout_calculations():
    ...
    test_cases = [
        ...
        (
            "Google Microsoft OpenAI AI services competition revenue market share 2024 2025",
            None,
            "Complex 12-word query",
        ),
        (
            "comprehensive analysis artificial intelligence market trends technology sector growth forecast",
            None,
            "11-word complex query",
        ),
        # Budget-constrained scenarios
        ("simple query", 15.0, "Simple query with 15s budget"),
        (
            "Google Microsoft OpenAI competition analysis",
            20.0,
            "Complex query with limited budget",
        ),
        (
            "Google Microsoft OpenAI AI services competition revenue market share 2024 2025",
            40.0,
            "Complex query with good budget",
        ),
        # Very tight budget scenarios (should enforce minimum)
        (
            "Google Microsoft OpenAI AI services competition revenue market share 2024 2025",
            5.0,
            "Complex query with very tight budget",
        ),
    ]
...
async def test_failure_tolerance():
    ...
    print(
        f"   Circuit breaker threshold: {getattr(settings.performance, 'search_circuit_breaker_failure_threshold', 8)}"
    )
    print(
        f"   Circuit breaker recovery: {getattr(settings.performance, 'search_circuit_breaker_recovery_timeout', 30)}s"
    )
...
async def main():
    ...
    try:
        ...
        print(
            "   ✅ Tolerant failure handling: 12 timeout failures vs 6 other failures"
        )
