...
class SpeedBenchmarkRunner:
        ...
        results["model_selection"].append(
            {
                ...
                "is_fast_model": emergency_config.model_id
    ...
    async def run_full_benchmark(self) -> dict[str, Any]:
        ...
        for complexity in QueryComplexity:
            ...
            for query in queries[:2]:  # Test 2 queries per complexity
                with SpeedTestMonitor(
                    f"full_{complexity.value}", complexity
                ) as monitor:
                    if complexity == QueryComplexity.EMERGENCY:
                        result = await validator.test_emergency_mode_performance(query)
                    else:
        ...
        results["speedup_analysis"] = {
            ...
            "meets_2x_threshold": all(
                r["speedup_factor"] >= 2.0 for r in speedup_results
            ),
            "meets_3x_threshold": all(
                r["speedup_factor"] >= 3.0 for r in speedup_results
            ),
        }
        ...
        model_tests = []
        for time_budget, expected_speed_tier in [
            (8, "ultra_fast"),
            (20, "fast"),
            (60, "balanced"),
        ]:
    ...
    async def run_emergency_mode_benchmark(self) -> dict[str, Any]:
        ...
        for query in emergency_queries:
            ...
            with SpeedTestMonitor(
                f"emergency_{total_tests}", QueryComplexity.EMERGENCY
            ):
                result = await validator.test_emergency_mode_performance(query)
        ...
        results["timeout_compliance"] = {
            ...
            "passes_threshold": (timeout_failures / max(total_tests, 1))
            <= SPEED_THRESHOLDS["timeout_failure_threshold"],
        }
                ...
                results["comparisons"].append(
                    {
                        ...
                        "speedup_factor": result["speedup_factor"],
                        "improvement_percent": (
                            (result["baseline_time"] - result["optimized_time"])
    ...
    def generate_report(self, test_results: dict[str, Any]) -> str:
        ...
        if "emergency_mode" in test_results:
            ...
            if emergency_results:
                avg_time = statistics.mean(
                    [r["execution_time"] for r in emergency_results]
                )
            ...
            if speedup_results:
                for result in speedup_results:
                    if "result" in result and "speedup_factor" in result["result"]:
                        ...
                        report.append(
                            f"- {result['complexity']} speedup: {speedup:.2f}x"
                        )
            report.append("")
        ...
        if "complexity_tests" in test_results:
            ...
            for complexity, results in test_results["complexity_tests"].items():
                if results:
                    ...
                    report.append(
                        f"- {complexity.title()}: {avg_time:.2f}s (threshold: {threshold}s) {passes}"
                    )
            report.append("")
        ...
        if "emergency_tests" in test_results:
            ...
            if emergency_tests:
                avg_time = statistics.mean(
                    [t["execution_time"] for t in emergency_tests]
                )
                success_rate = sum(1 for t in emergency_tests if t["success"]) / len(
                    emergency_tests
                )
                ...
                if "timeout_compliance" in test_results:
                    ...
                    passes_compliance = (
                        "✅" if compliance.get("passes_threshold", False) else "❌"
                    )
                    report.append(
                        f"- Timeout compliance: {passes_compliance} ({compliance.get('failure_rate', 0):.1%} failure rate)"
                    )
            report.append("")
        ...
        if "comparisons" in test_results:
            ...
            for comp in test_results["comparisons"]:
                report.append(
                    f"- {comp['complexity'].title()}: {comp['baseline_time']:.2f}s → {comp['optimized_time']:.2f}s ({comp['speedup_factor']:.2f}x speedup)"
                )

            if "summary" in test_results:
                summary = test_results["summary"]
                report.append(
                    f"- Overall average speedup: {summary['average_speedup']:.2f}x"
                )
            report.append("")
        ...
        if "speedup_analysis" in test_results:
            ...
            if not analysis.get("meets_2x_threshold", True):
                recommendations.append(
                    "🔧 Investigate speed optimizations - not meeting 2x speedup target"
                )
            elif analysis.get("meets_3x_threshold", False):
                recommendations.append(
                    "🎉 Excellent performance - exceeding 3x speedup target!"
                )
            else:
                recommendations.append(
                    "📈 Good performance - meeting 2x target, opportunity for 3x"
                )
    ...
    def save_results(self, test_results: dict[str, Any], mode: str):
        ...
        json_file = (
            self.output_dir
            / f"speed_benchmark_{mode}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        md_file = (
            self.output_dir
            / f"speed_benchmark_{mode}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
...
async def main():
    ...
    parser.add_argument("--query", type=str, help="Custom query to benchmark")
    parser.add_argument(
        "--output-dir",
        type=Path,
