...
def demonstrate_adaptive_model_selection():
    ...
    for scenario_name, time_budget, complexity in scenarios:
        ...
        speed_tier = (
            "🚀 ULTRA-FAST"
            if config.model_id in ["google/gemini-2.5-flash"]
            else "⚡ FAST"
            if config.model_id in ["openai/gpt-4o-mini"]
            else "🎯 BALANCED"
            if config.model_id in ["anthropic/claude-sonnet-4"]
            else "🧠 COMPREHENSIVE"
        )

        logger.info(f"{scenario_name:25} → {config.model_id:30} {speed_tier}")
        logger.info(
            f"{'':25}   Timeout: {config.timeout_seconds}s, Tokens: {config.max_tokens}"
        )
        logger.info("")
...
def demonstrate_progressive_token_budgeting():
    ...
    for scenario_name, total_budget in scenarios:
        budgeter = ProgressiveTokenBudgeter(
            total_time_budget_seconds=total_budget, confidence_target=0.75
        )
...
def demonstrate_complexity_calculation():
    ...
    test_queries = [
        ...
        (
            "Moderate Query",
            "Apple Inc Q4 earnings analysis with revenue growth and market outlook",
            TaskType.MARKET_ANALYSIS,
        ),
        (
            "Complex Query",
            "Apple Inc comprehensive financial analysis including EBITDA margins, cash flow assessment, competitive position versus Samsung and Google, supply chain risks, and forward P/E ratios with DCF valuation model",
    ...
    for query_type, query_text, task_type in test_queries:
        ...
        logger.info(
            f"{'':15}   Query: {query_text[:60]}{'...' if len(query_text) > 60 else ''}"
        )
        logger.info("")
...
async def demonstrate_speed_improvements():
    ...
    for query_type, baseline, optimized, speedup in scenarios:
        logger.info(
            f"{query_type:16} │ {baseline:15} │ {optimized:15} │ {speedup:.1f}x speedup"
        )
