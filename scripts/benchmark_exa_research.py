...
from maverick_mcp.agents.deep_research import DeepResearchAgent, RESEARCH_DEPTH_LEVELS
...
class ExaResearchBenchmark:
    ...
    async def run_single_benchmark(
        ...
        print(f"🔍 Testing: {query[:30]}... | Depth: {depth} | Focus: {focus_area} | Parallel: {parallel}")
    ...
    async def benchmark_parallel_vs_sequential(self, query: str, depth: str = "standard") -> Dict[str, Any]:
        ...
        speedup = 1.0
        if (sequential_result.get("status") == "success" and 
            parallel_result.get("status") == "success"):
        ...
        return {
            ...
            "speedup": speedup,
            "parallel_efficiency": parallel_result.get("parallel_stats", {}).get("parallel_efficiency", 1.0),
        }
    
    async def benchmark_focus_areas(self, query: str, depth: str = "standard") -> List[Dict[str, Any]]:
        ...
        for timeout in timeout_budgets:
            ...
            timeout_results.append({
                **result,
                ...
                "timed_out": result.get("status") in ["timeout", "error"] and "timeout" in str(result.get("error", "")),
            })
    ...
    async def run_comprehensive_benchmark(
        ...
        print(f"⚙️  Configuration: Depths={depths or 'all'}, Focus={focus_areas or 'all'}, Parallel={include_parallel}")
        ...
        selected_depths = depths or (["basic", "standard"] if quick_mode else self.research_depths)
        ...
        if include_parallel:
            ...
            for query in test_queries[:2]:
                for depth in selected_depths[:2]:  # Test first 2 depths
                    comparison = await self.benchmark_parallel_vs_sequential(query, depth)
    ...
    def generate_summary_report(self, total_time: float):
        ...
        successful_results = [r for r in self.benchmark_results if r.get("status") == "success"]
        failed_results = [r for r in self.benchmark_results if r.get("status") in ["failed", "error"]]
        ...
        if successful_results:
            ...
            print(f"   Min/Max Time: {min(execution_times):.2f}s / {max(execution_times):.2f}s")
        ...
        parallel_results = [r for r in self.benchmark_results if r.get("benchmark_type") == "parallel_comparison"]
        if parallel_results:
    ...
    def save_detailed_results(self):
        ...
        summary_data = {
            "benchmark_info": {
                ...
                "successful_tests": len([r for r in self.benchmark_results if r.get("status") == "success"]),
                "failed_tests": len([r for r in self.benchmark_results if r.get("status") in ["failed", "error"]]),
            },
            "configuration": {
    ...
    def save_csv_summary(self, csv_file: Path):
        ...
        fieldnames = [
            "timestamp", "query", "depth", "focus_area", "parallel", 
            "execution_time", "timeout_budget", "sources_analyzed", 
            "confidence_score", "key_insights_count", "citations_count"
        ]
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark ExaSearch research integration")
