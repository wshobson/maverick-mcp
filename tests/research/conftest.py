...
`maverick.market_data.config` settings singleton -- the research layer
...
`maverick.research.config` is still docstring-only at this stage (Task 0);
once it defines `ResearchSettings`/`reset_research_settings` in Task 3, this
fixture gains the same reset call the sibling domains' conftests make for
their own settings.
"""
