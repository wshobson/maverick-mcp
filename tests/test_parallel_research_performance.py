...
class PerformanceMonitor:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        avg_cpu = statistics.mean(self.cpu_percent_samples) if self.cpu_percent_samples else 0
...
class MockResearchExecutor:
    ...
    def _generate_research_response(self, task: ResearchTask) -> dict[str, Any]:
        ...
        sources = [
            {
                ...
                "content_snippet": f"Content from source {i+1} " * random.randint(10, 50),
        ...
        return {
            ...
            "insights": insights,
            "risk_factors": [f"{task.task_type} risk factor {i+1}" for i in range(random.randint(3, 7))],
            "opportunities": [f"{task.task_type} opportunity {i+1}" for i in range(random.randint(3, 7))],
            "sentiment": {
            ...
            "detailed_analysis": {
                f"analysis_point_{i}": f"Detailed analysis content {i} " * random.randint(50, 200)
    ...
    def get_execution_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "task_type_distribution": {
                task_type: len([log for log in self.execution_log if log["task_type"] == task_type])
                for task_type in ["fundamental", "technical", "sentiment", "competitive"]
            }
        }
...
class PerformanceTester:
    ...
    @staticmethod
    def create_test_tasks(agent_count: int, session_id: str = "perf_test") -> list[ResearchTask]:
    ...
    @staticmethod
    async def run_sequential_baseline(tasks: list[ResearchTask], 
                                    executor: MockResearchExecutor) -> dict[str, Any]:
    ...
    @staticmethod
    async def run_parallel_test(tasks: list[ResearchTask], 
                              config: ParallelResearchConfig,
                              executor: MockResearchExecutor) -> ResearchResult:
        ...
        async def mock_synthesis(task_results: dict[str, ResearchTask]) -> dict[str, Any]:
            ...
            return {
                ...
                "key_findings": [f"Finding {i}" for i in range(min(len(successful_results), 5))],
            }
...
class TestParallelResearchPerformance:
    ...
    @pytest.mark.parametrize("failure_rate", FAILURE_RATES[:2])  # 0% and 10% for unit tests
    async def test_parallel_speedup_thresholds(self, 
                                             agent_count: int,
                                             failure_rate: float,
                                             performance_monitor,
        ...
        for run in range(STATISTICAL_RUNS):
            with performance_monitor(f"{test_name}_run{run}") as monitor:
                ...
                tasks = performance_tester.create_test_tasks(agent_count, f"speedup_test_{run}")
                ...
                sequential_result = await performance_tester.run_sequential_baseline(tasks, executor)
                ...
                parallel_result = await performance_tester.run_parallel_test(tasks, config, executor)
        ...
        assert all(m < 10.0 for m in memory_results), "Memory usage should be reasonable"
    ...
    async def test_performance_under_failures(self,
        ...
        for failure_rate in FAILURE_RATES:
            with performance_monitor(f"{test_name}_{int(failure_rate*100)}pct") as monitor:
                config = ParallelResearchConfig(
                    max_concurrent_agents=agent_count,
                    timeout_per_agent=3,
                    enable_fallbacks=True,
                ...
                tasks = performance_tester.create_test_tasks(agent_count, f"failure_test_{int(failure_rate*100)}")
                executor = mock_executor_factory(
                    failure_rate=failure_rate,
                    base_delay=0.05,
            ...
            results[failure_rate] = {
                ...
                "success_rate": parallel_result.successful_tasks / (parallel_result.successful_tasks + parallel_result.failed_tasks)
            }
        
        # Validate failure handling - adjusted for realistic expectations
        assert results[0.0]["success_rate"] == 1.0, "Zero failure rate should achieve 100% success"
        assert results[0.10]["success_rate"] >= 0.7, "10% failure rate should maintain >70% success"
        assert results[0.25]["success_rate"] >= 0.5, "25% failure rate should maintain >50% success"
        
        # Validate performance doesn't degrade drastically with failures
        baseline_time = results[0.0]["execution_time"]
        assert results[0.10]["execution_time"] <= baseline_time * 1.5, "10% failure shouldn't increase time by >50%"
        assert results[0.25]["execution_time"] <= baseline_time * 2.0, "25% failure shouldn't double execution time"
    ...
    async def test_memory_usage_patterns(self,
        ...
        for agent_count in [1, 2, 4]:
            with performance_monitor(f"memory_test_{agent_count}_agents") as monitor:
                ...
                tasks = performance_tester.create_test_tasks(agent_count * 2, f"memory_test_{agent_count}")
                executor = mock_executor_factory(
                    failure_rate=0.0,
                ...
                result = await performance_tester.run_parallel_test(tasks, config, executor)
...
class TestParallelResearchIntegrationPerformance:
    ...
    async def test_deep_research_agent_parallel_integration(self,
                                                          performance_monitor,
                                                          mock_executor_factory):
        ...
        assert execution_time_seconds < 10.0, "Integration test should complete within 10s"
        assert monitor.metrics["memory_multiplier"] < 5.0, "Memory usage should be reasonable"
        
        logger.info("Deep research agent integration test passed")

    @pytest.mark.parametrize("failure_rate", [0.25])  # Only high failure rate for slow tests
    async def test_high_failure_rate_resilience(self,
        ...
        for run in range(STATISTICAL_RUNS):
            with performance_monitor(f"{test_name}_run{run}") as monitor:
                ...
                tasks = performance_tester.create_test_tasks(agent_count, f"resilience_test_{run}")
                executor = mock_executor_factory(
                    failure_rate=failure_rate,
                ...
                try:
                    ...
                    success_rate = result.successful_tasks / (result.successful_tasks + result.failed_tasks)
        ...
        avg_success_rate = statistics.mean([r["success_rate"] for r in resilience_results])
        avg_execution_time = statistics.mean([r["execution_time"] for r in resilience_results])
        ...
        assert avg_execution_time < 20.0, "High failure rate tests should still complete reasonably"
...
class TestSuitePerformance:
    ...
    async def test_total_test_duration_under_threshold(self,
                                                     performance_monitor):
        ...
        assert monitor.metrics["execution_time"] < 1.0, "Meta-test should complete quickly"
        assert monitor.metrics["memory_multiplier"] < 2.0, "Meta-test should use minimal memory"
    ...
    def test_performance_threshold_configuration(self):
        ...
        assert PERFORMANCE_THRESHOLDS["speedup"][2] > 1.0, "2-agent speedup should exceed 1x"
        assert PERFORMANCE_THRESHOLDS["speedup"][4] > PERFORMANCE_THRESHOLDS["speedup"][2], "4-agent speedup should exceed 2-agent"
        assert PERFORMANCE_THRESHOLDS["memory_multiplier"] > 1.0, "Memory multiplier should allow some increase"
        assert PERFORMANCE_THRESHOLDS["memory_multiplier"] < 10.0, "Memory multiplier should be reasonable"
        assert PERFORMANCE_THRESHOLDS["max_test_duration"] > 10.0, "Test duration budget should be reasonable"
        
        # Test failure rate configuration
        assert all(0.0 <= rate <= 1.0 for rate in FAILURE_RATES), "Failure rates should be valid percentages"
        assert len(set(FAILURE_RATES)) == len(FAILURE_RATES), "Failure rates should be unique"
        
        # Test statistical significance configuration
        assert STATISTICAL_RUNS >= 3, "Statistical runs should provide reasonable sample size"
