...
class TestDeepResearchAgentParallelExecution:
    ...
    def mock_llm(self):
        """Create mock LLM."""
        return MockLLM('{"KEY_INSIGHTS": ["Test insight"], "SENTIMENT": {"direction": "bullish", "confidence": 0.8}, "CREDIBILITY": 0.9}')
    ...
    async def test_parallel_execution_mode_selection(self, deep_research_agent):
        ...
        with patch.object(deep_research_agent, '_execute_parallel_research') as mock_parallel, \
    ...
    async def test_sequential_execution_mode_selection(self, sequential_agent):
        ...
        with patch.object(sequential_agent, '_execute_parallel_research') as mock_parallel, \
    ...
    @pytest.mark.asyncio
    async def test_execute_parallel_research_task_distribution(self, deep_research_agent):
        """Test parallel research task distribution."""
        with patch.object(deep_research_agent.task_distributor, 'distribute_research_tasks') as mock_distribute, \
             patch.object(deep_research_agent.parallel_orchestrator, 'execute_parallel_research') as mock_execute:
            
            # Mock task distribution
            mock_tasks = [
                ResearchTask("test_123_fundamental", "fundamental", "AAPL", ["earnings"]),
    ...
    async def test_subagent_task_routing(self, deep_research_agent):
        ...
        fundamental_task = ResearchTask("test_fundamental", "fundamental", "AAPL", ["earnings"])
        
        with patch.object(deep_research_agent, 'FundamentalResearchAgent') as mock_fundamental:
            mock_subagent = AsyncMock()
            mock_subagent.execute_research.return_value = {"research_type": "fundamental"}
            ...
            with patch('maverick_mcp.agents.deep_research.FundamentalResearchAgent', mock_fundamental):
                result = await deep_research_agent._execute_subagent_task(fundamental_task)
    ...
    async def test_unknown_task_type_fallback(self, deep_research_agent):
        ...
        with patch('maverick_mcp.agents.deep_research.FundamentalResearchAgent') as mock_fundamental:
            mock_subagent = AsyncMock()
            mock_subagent.execute_research.return_value = {"research_type": "fundamental"}
            mock_fundamental.return_value = mock_subagent
    ...
    async def test_parallel_result_synthesis(self, deep_research_agent, mock_llm):
        ...
        task_results = {
            "test_123_fundamental": ResearchTask("test_123_fundamental", "fundamental", "AAPL", ["earnings"]),
            "test_123_sentiment": ResearchTask("test_123_sentiment", "sentiment", "AAPL", ["news"]),
        }
    ...
    async def test_synthesis_with_mixed_results(self, deep_research_agent):
        ...
        task_results = {
            "test_123_fundamental": ResearchTask("test_123_fundamental", "fundamental", "AAPL", ["earnings"]),
            "test_123_technical": ResearchTask("test_123_technical", "technical", "AAPL", ["charts"]),
            "test_123_sentiment": ResearchTask("test_123_sentiment", "sentiment", "AAPL", ["news"]),
        }
    ...
    async def test_synthesis_llm_failure_fallback(self, deep_research_agent):
        ...
        # Mock LLM to fail
        with patch.object(deep_research_agent.llm, 'ainvoke', side_effect=RuntimeError("LLM failed")):
            result = await deep_research_agent._synthesize_parallel_results(task_results)
    ...
    async def test_format_parallel_research_response(self, deep_research_agent):
        ...
        task1 = ResearchTask("test_123_fundamental", "fundamental", "AAPL", ["earnings"])
    ...
    async def test_parallel_recommendation_derivation(self, deep_research_agent):
        ...
        recommendation = deep_research_agent._derive_parallel_recommendation(bullish_sentiment)
        ...
        recommendation = deep_research_agent._derive_parallel_recommendation(bearish_sentiment)
        assert "caution" in recommendation.lower() or "negative" in recommendation.lower()
        
        # Test neutral/mixed signals
        neutral_sentiment = {"direction": "neutral", "confidence": 0.5}
        recommendation = deep_research_agent._derive_parallel_recommendation(neutral_sentiment)
...
class TestSpecializedSubagents:
    ...
    async def test_fundamental_research_agent(self, mock_parent_agent):
        ...
        mock_parent_agent.content_analyzer.analyze_content = AsyncMock(return_value={
            "insights": ["Strong earnings growth"],
            "sentiment": {"direction": "bullish", "confidence": 0.8},
        ...
        with patch.object(subagent, '_perform_specialized_search') as mock_search:
            ...
            task = ResearchTask("fund_task", "fundamental", "AAPL analysis", ["earnings"])
    ...
    def test_technical_query_generation(self, mock_parent_agent):
        ...
        assert any("rsi" in query.lower() or "macd" in query.lower() for query in queries)
    ...
    async def test_competitive_research_agent(self, mock_parent_agent):
        ...
        with patch.object(subagent, '_perform_specialized_search') as mock_search:
            ...
            task = ResearchTask("comp_task", "competitive", "AAPL analysis", ["market_share"])
            result = await subagent.execute_research(task)
...
class TestDeepResearchParallelIntegration:
    ...
    def integration_agent(self):
        ...
        llm = MockLLM('{"KEY_INSIGHTS": ["Integration insight"], "SENTIMENT": {"direction": "bullish", "confidence": 0.8}}')
    ...
    async def test_end_to_end_parallel_research(self, integration_agent):
        ...
        with patch.object(integration_agent, '_execute_subagent_task') as mock_execute:
            ...
            assert result["execution_mode"] == "parallel"
            assert result["research_topic"] == "Apple Inc comprehensive financial analysis"
    ...
    async def test_parallel_vs_sequential_performance(self, integration_agent):
        ...
        with patch.object(integration_agent, '_execute_subagent_task', side_effect=mock_subagent_execution):
            ...
            if "parallel_execution_stats" in parallel_result:
                assert parallel_result["parallel_execution_stats"]["parallel_efficiency"] > 0
    ...
    async def test_research_quality_consistency(self, integration_agent):
        ...
        mock_response = {
            ...
            "credibility_score": 0.8,
            "sources": [{"title": "Source", "url": "https://example.com", "credibility_score": 0.8}]
        }
        
        with patch.object(integration_agent, '_execute_subagent_task', return_value=mock_response):
            parallel_result = await integration_agent.research_comprehensive(
                topic=topic,
