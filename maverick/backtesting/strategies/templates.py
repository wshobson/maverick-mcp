"""Rule-based strategy catalog. Third-layer sibling: imports config and types.

Ports `STRATEGY_TEMPLATES` and its three accessors from
`maverick_mcp/backtesting/strategies/templates.py` verbatim (parameter
schemas, optimization ranges, and the descriptive `code` strings are copied
character-for-character). Two things from that legacy module are NOT
ported:

- `SimpleMovingAverageStrategy`: a standalone `Strategy` subclass used only
  by `maverick_mcp/api/routers/backtesting.py` as a base-strategy building
  block for ML ensemble/adaptive tools (`create_strategy_ensemble`,
  `run_ml_strategy_backtest`). It is not part of `STRATEGY_TEMPLATES` and
  has zero callers anywhere in the already-ported `maverick/` tree
  (confirmed by `grep` across `maverick/backtesting/strategies/ml/`). It is
  a tools-tier concern, out of scope for this data-tier module -- Task 7's
  service layer can add it if an ML-ensemble tool ends up needing it.
- `StrategyParser` (`parser.py`): natural-language-to-strategy-config
  parsing via `langchain_anthropic`. Not a "rule-based template" in the
  sense this module ports -- it's a separate LLM-backed concern with a
  dependency (`langchain_anthropic`) outside this layer's allowed imports.

Catalog count correction: the task brief describes "13 rule-based
templates." Parsing `STRATEGY_TEMPLATES` in the legacy file
(`ast.parse` + walking the dict's keys) shows exactly 12 top-level entries:
sma_cross, rsi, macd, bollinger, momentum, ema_cross, mean_reversion,
breakout, volume_momentum, online_learning, regime_aware, ensemble. There
is no 13th entry or hidden alias at the catalog level (`sma_crossover` is
only a dispatch-time alias for `sma_cross` inside `signals.py`'s
`generate_signals`, not a separate catalog key). All 12 are ported here
verbatim.

Of those 12, three (`online_learning`, `regime_aware`, `ensemble`) carry a
`code` field that is a descriptive comment rather than an executable
vectorbt-expression string -- these are the "stub" templates referenced in
the task brief, at the *catalog-display* level. See `signals.py`'s module
docstring for why their actual runtime signal generation is nonetheless a
real, executable implementation (not a delegation to the ML strategy
classes in `strategies/ml/`).

`get_strategy_info` returns `maverick.backtesting.types.StrategyCatalogEntry`
(built for exactly this field set) rather than the legacy's raw dict, matching
the typed-return convention already established by `analysis.py`. The
`categories` grouping and `total_count` aggregation that the legacy
`list_strategies` *router* builds around `get_strategy_info` are a
tools-tier concern (they live in
`maverick_mcp/api/routers/backtesting.py`, not in `templates.py`) and are
therefore also out of scope here -- `StrategyCatalog` (which bundles them)
is Task 7's to assemble.
"""
...
def get_strategy_info(strategy_type: str) -> StrategyCatalogEntry:
